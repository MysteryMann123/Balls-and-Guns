...
export function ballShooting(game, now) {
    for (const shooter of game.balls) {
        ...
        if (shooter.isPlayerControlled || shooter.isTrainingDummy) continue;
...
export function fireControlledWeapon(game, shooter, target, now) {
    ...
    if (target.isUntargetable(now)) return false;

    shooter.weapon.updateReload(now);
    if (shooter.isShootLocked(now)) return false;
    if (now < shooter.nextShootAllowedAt) return false;
    ...
    if (shooter.weapon.type === 'flamethrower') {
        ...
        const dy = target.pos.y - shooter.pos.y;
        const distanceToTarget = Math.sqrt(dx * dx + dy * dy);
        if (distanceToTarget > FLAMETHROWER_HITSCAN_RANGE) return false;
    }

    if (shooter.weapon.type === 'musket') {
        tryMusketBayonet(game, shooter, now);
    }

    if (shooter.weapon.type === 'penitence') {
        if (!shooter.weapon.shoot(now)) return false;
        shooter.penitence.swingStartedAt = now;
        shooter.penitence.swingUntil = now + PENITENCE_SWING_ANIMATION_MS;
        tryPenitenceSwing(game, shooter, now);
        shooter.nextShootAllowedAt = now + shooter.reactionDelayMs;
    ...
    if (shooter.weapon.type === 'hornet') {
        ...
        if (!shooter.weapon.shoot(now, hornetForm)) return false;
    ...
    if (shooter.weapon.type === 'dealer') {
        ...
        shooter.dealerWeaponState.fire(shooter, [target], game.projectiles);
        shooter.nextShootAllowedAt = now + shooter.reactionDelayMs;
        return true;
    }
    ...
    for (let p = 0; p < shooter.weapon.pelletsPerShot; p++) {
        ...
        if (shooter.weapon.type === 'blutsauger') {
            angle += (Math.random() * 2 - 1) * shooter.weapon.spreadAngle;
        }
        if (shooter.weapon.type === 'tommygun') {
            angle += (Math.random() * 2 - 1) * shooter.weapon.spreadAngle;
        }
        ...
        if (shooter.weapon.type === 'beggersbazooka') {
            angle += (Math.random() * 2 - 1) * BEGGERS_BAZOOKA_DEVIATION;
        }
        if (shooter.weapon.type === 'flamethrower' || shooter.weapon.type === 'grenadelauncher' || shooter.weapon.type === 'piplauncher' || shooter.weapon.type === 'musket' || shooter.weapon.type === 'egomagicbullet' || shooter.weapon.type === 'paradiselost') {
            angle += (Math.random() * 2 - 1) * shooter.weapon.spreadAngle;
        }
        if (shooter.weapon.type === 'rocketjumper') {
            angle += (Math.random() * 2 - 1) * 0.02 + Math.PI;
        }
        ...
        if (shooter.weapon.type === 'rocketjumper') {
            const blastDirection = new Vector(Math.cos(angle), Math.sin(angle));
            shooter.applyImpulse(blastDirection.clone().multiply(-ROCKET_JUMPER_SELF_BLAST_IMPULSE));
        }
